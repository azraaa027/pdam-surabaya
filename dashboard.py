...
def load_data():
    ...
    # Jittering agar titik tidak menumpuk sempurna
...
if df.empty:
    st.error("File 'Data_Siap_Clustering_Final3.csv' tidak ditemukan.")
    st.stop()
...
if jenis_periode == "Semua Waktu (Default)":
    ...
    # Untuk "Semua Waktu", kita ambil rata-rata pelanggan bulanan per jalan agar tidak double counting (sum)
    df_filtered = df_selection.groupby(['JALAN', 'Latitude', 'Longitude', 'Lat_Jitter', 'Lon_Jitter']).agg({
        'TOTAL_PAKAI': 'sum',
        'TOTAL_RP': 'sum',
    ...
    df_selection = df[df['TAHUN'] == pilih_waktu].copy()
    df_filtered = df_selection.groupby(['JALAN', 'Latitude', 'Longitude', 'Lat_Jitter', 'Lon_Jitter']).agg({
        'TOTAL_PAKAI': 'sum',
        'TOTAL_RP': 'sum',
        'JUMLAH_PELANGGAN': 'mean'
    ...
    df_selection = df[df['NAMA_BULAN'] == pilih_waktu].copy()
    df_filtered = df_selection.copy() # Sudah per bulan, tidak perlu agg tambahan
...
with col_l1:
    if os.path.exists("Badge_ITS.png"): st.image("Badge_ITS.png", use_container_width=True)
with col_l2:
    if os.path.exists("logo-statistika-white-border.png"): st.image("logo-statistika-white-border.png", use_container_width=True)

# ==========================================
# 4. TOP METRICS (Logika Akal-akalan)
# ==========================================
total_pakai = df_selection['TOTAL_PAKAI'].sum()
total_rp = df_selection['TOTAL_RP'].sum()

# LOGIKA: Ambil rata-rata pelanggan per jalan, lalu jumlahkan totalnya.
# Ini mencegah ID yang sama dihitung berkali-kali selama 26 bulan.
pelanggan_per_jalan = df_selection.groupby('JALAN')['JUMLAH_PELANGGAN'].mean().reset_index()
total_id_unik = pelanggan_per_jalan['JUMLAH_PELANGGAN'].sum()
...
with map_col1:
    ...
    fig_map1 = px.scatter_mapbox(df_filtered, lat="Lat_Jitter", lon="Lon_Jitter", color='TOTAL_PAKAI', size='TOTAL_PAKAI',
                                 ...
                                 mapbox_style="carto-positron", labels={'TOTAL_PAKAI': 'm³'})
...
with map_col2:
    ...
    fig_map2 = px.scatter_mapbox(df_filtered, lat="Lat_Jitter", lon="Lon_Jitter", color='TOTAL_RP', size='TOTAL_RP',
                                 color_continuous_scale=["#2ecc71", "#f1c40f", "#e74c3c"], hover_name="JALAN",          
                                 hover_data=hover_konf, zoom=13.5, center={"lat": -7.285, "lon": 112.796}, 
                                 mapbox_style="carto-positron", labels={'TOTAL_RP': 'Rp'})
...
# ==========================================
st.divider()
st.subheader(f"🏆 Top 10 Jalan Berdasarkan Pemakaian Air dan Total Tagihan ({label_waktu})")
...
with c1:
    ...
    df_top_p = df_tabel[['JALAN', 'TOTAL_PAKAI']].sort_values(by='TOTAL_PAKAI', ascending=False).head(10)
    df_top_p['TOTAL_PAKAI'] = df_top_p['TOTAL_PAKAI'].apply(lambda x: f"{int(x):,}".replace(',', '.'))
    st.dataframe(df_top_p, hide_index=True, use_container_width=True)

with c2:
    fig_p = px.bar(df_tabel.sort_values(by='TOTAL_PAKAI', ascending=False).head(5).sort_values(by='TOTAL_PAKAI', ascending=True), 
                   x='TOTAL_PAKAI', y='JALAN', orientation='h', text='TOTAL_PAKAI', color_discrete_sequence=['#3498db'])
...
with c3:
    ...
    df_top_r = df_tabel[['JALAN', 'TOTAL_RP']].sort_values(by='TOTAL_RP', ascending=False).head(10)
    df_top_r['TOTAL_RP'] = df_top_r['TOTAL_RP'].apply(lambda x: f"Rp {int(x):,}".replace(',', '.'))
    st.dataframe(df_top_r, hide_index=True, use_container_width=True)

with c4:
    fig_r = px.bar(df_tabel.sort_values(by='TOTAL_RP', ascending=False).head(5).sort_values(by='TOTAL_RP', ascending=True), 
                   x='TOTAL_RP', y='JALAN', orientation='h', text='TOTAL_RP', color_discrete_sequence=['#2ecc71'])
...
with col_t_h:
    ...
    df_top_h = df_tabel[['JALAN', 'HARGA_PER_M3']].sort_values(by='HARGA_PER_M3', ascending=False).head(10)
    df_top_h['HARGA_PER_M3'] = df_top_h['HARGA_PER_M3'].apply(lambda x: f"Rp {int(x):,}".replace(',', '.'))
    st.dataframe(df_top_h, hide_index=True, use_container_width=True)
with col_b_h:
    fig_h = px.bar(df_tabel.sort_values(by='HARGA_PER_M3', ascending=False).head(5).sort_values(by='HARGA_PER_M3', ascending=True), 
                   x='HARGA_PER_M3', y='JALAN', orientation='h', text='HARGA_PER_M3', color_discrete_sequence=['#9b59b6'])
